...
// swap table
__device__ __managed__ u32 swaptable[2*STORAGE_SIZE/PAGE_SIZE]; // 1 for empty, 1 for page number
...
__global__ void mykernel(int input_size) {
  ...
  __shared__ uchar data[PHYSICAL_MEM_SIZE];
    ...
    for (int i=0;i<4;i++){
      // the thread id is 0,1,2,3
      // each time we run the program, we let each thread to run the same task
      if (threadIdx.x == i){
        VirtualMemory vm;
        vm_init(&vm, data, storage, pt, swaptable, &pagefault_num, PAGE_SIZE,
